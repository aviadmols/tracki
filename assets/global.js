...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!thumbnailList || !prevButton || !nextButton) return;

  function getThumbnails() {
    return Array.from(
      thumbnailList.querySelectorAll('.thumbnail-list__item .thumbnail')
    );
  }
  ...
  function activateThumbnail(index) {
    ...
    // Trigger Harmony's existing thumbnail functionality
    ...
    // Scroll thumbnail into view vertically
  ...
  prevButton.addEventListener(
    ...
    function (event) {
      ...
      activateThumbnail(currentIndex - 1);
    },
    true
  );
