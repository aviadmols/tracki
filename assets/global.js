...
/** Ensures UTM / click ids are on the cart before add-to-cart (see utm-order-attribution.js). */
theme.trackiAwaitAttributionSync = function () {
  var ta = window.TrackiAttribution;
  if (ta && typeof ta.syncCart === 'function') {
    return Promise.resolve(ta.syncCart());
  }
  ...
  onClickHandler() {
    ...
    if (variantId) {
      ...
      if (document.body.classList.contains('template-cart') || !theme.shopSettings.cartDrawer) {
        theme.trackiAwaitAttributionSync().then(postLink);
        return;
      }

      theme.trackiAwaitAttributionSync().then(runAjaxAdd);
    }
  }
}
...
class BundleProducts extends HTMLElement {
  ...
  onButtonClick(event) {
    ...
    if (document.body.classList.contains('template-cart') || !theme.shopSettings.cartDrawer) {
      theme.trackiAwaitAttributionSync().then(postLinkBundle);
      return;
    }

    theme.trackiAwaitAttributionSync().then(runAjaxBundle);
  }
...
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('thumbnail-slider').forEach(function (slider) {
    ...
    function getThumbnails() {
      return Array.from(
        list.querySelectorAll('.thumbnail-list__item .thumbnail')
      );
    }
    ...
    function getCurrentIndex() {
      ...
      const active = thumbnails.findIndex(function (thumbnail) {
        return thumbnail.getAttribute('aria-current') === 'true';
      });

      return active >= 0 ? active : currentIndex;
    }
    ...
    function goToImage(index) {
      ...
      /* Click Dawn's thumbnail button */
      thumbnails[index].click();

      /* Scroll the thumbnail into view */
      const item = thumbnails[index].closest('.thumbnail-list__item');
