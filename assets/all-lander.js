/* ============================================================
   main.js — Tracki Parents interactions
   ============================================================ */
...
if (video && playBtn && videoWrap) {
  ...
  // End of video resets button
  video.addEventListener('ended', () => {
    playBtn.classList.remove('hidden');
  });

  // Scroll into view logic (plays once)
  let hasPlayed = false;
  const vidObs = new IntersectionObserver(([e]) => {
    if (e.isIntersecting && !hasPlayed) {
      hasPlayed = true;
      // Browsers restrict unmuted autoplay, so we play muted initially
      video.muted = true;
      video.play().then(() => {
        playBtn.classList.add('hidden');
      }).catch(err => {
